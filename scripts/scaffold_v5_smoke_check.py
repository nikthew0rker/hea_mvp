...
def main():
    ...
    elevated risk: 3-10
    '''
    extracted = extract_candidate_update(noisy)
    assert extracted["understood"]["topic"] == "diabetes"
    assert len(extracted["candidate_questions"]) >= 2
    assert extracted["candidate_scoring_rules"]["method"] == "sum_of_option_scores"
    assert extracted["candidate_risk_bands"], "expected noisy-text risk bands"

    graph = {
        "graph_id": "smoke_graph_v5",
        "title": "Smoke Graph V5",
        "topic": "smoke",
    ...
    print("scaffold v5 smoke check passed")
