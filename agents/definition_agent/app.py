...
app = FastAPI(title="Definition Agent", version="0.2.0")
...
def merge_draft_state(
    *,
    ...
    understood_current = current_draft.get("understood", {}) or {}
    understood_llm = llm_candidate.get("understood", {}) or {}
    ...
    questions = _merge_questions(
        current_draft.get("candidate_questions", []),
        llm_candidate.get("candidate_questions", []),
        heuristic.get("questions", []),
    )

    scoring_rules = (
        llm_candidate.get("candidate_scoring_rules")
        or current_draft.get("candidate_scoring_rules")
        or heuristic.get("scoring_logic")
        or {}
    )

    risk_bands = _merge_risk_bands(
        current_draft.get("candidate_risk_bands", []),
        llm_candidate.get("candidate_risk_bands", []),
        heuristic.get("risk_bands", []),
    )

    report_requirements = _merge_generic(
        current_draft.get("candidate_report_requirements", []),
        llm_candidate.get("candidate_report_requirements", []),
    )
    safety_requirements = _merge_generic(
        current_draft.get("candidate_safety_requirements", []),
        llm_candidate.get("candidate_safety_requirements", []),
    )
...
def _merge_questions(*sources: list) -> list[dict[str, Any]]:
...
def _merge_risk_bands(*sources: list) -> list[dict[str, Any]]:
    merged: list[dict[str, Any]] = []
...
def _merge_generic(*sources: list) -> list[dict[str, Any]]:
    merged: list[dict[str, Any]] = []
    seen: set[str] = set()
    for source in sources:
        if not source:
